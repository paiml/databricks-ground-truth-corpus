...
class PIIDetector:
    ...
    def detect(self, text: str) -> List[PIIMatch]:
        ...
        for m in self.INTL_PHONE_PATTERN.finditer(text):
            ...
            if not any(m.start() >= x.start and m.end() <= x.end
                      for x in matches if x.type == "phone_us"):
        ...
        for m in self.SSN_PATTERN.finditer(text):
            ...
            if not (value.startswith("9") or value.startswith("666") or
                    value.startswith("000") or value[3:5] == "00" or value[5:] == "0000"):
...
def get_email_test_cases() -> List[Tuple[str, List[Dict]]]:
    ...
    return [
        ...
        ("user+tag@gmail.com is valid", [
            {"type": "email", "value": "user+tag@gmail.com"}
        ]),
        # Negative cases
...
def get_phone_test_cases() -> List[Tuple[str, List[Dict]]]:
    ...
    return [
        ...
        ("Call me at 555-123-4567", [
            {"type": "phone_us", "value": "555-123-4567"}
        ]),
        ("Phone: (555) 123-4567", [
            {"type": "phone_us", "value": "(555) 123-4567"}
        ]),
        ("+1 555 123 4567 is my number", [
            {"type": "phone_us", "value": "+1 555 123 4567"}
        ]),
        # Negative cases
...
def get_credit_card_test_cases() -> List[Tuple[str, List[Dict]]]:
    ...
    return [
        ...
        ("MC: 5500000000000004", [  # MasterCard test
...
def run_pii_tests(detector: PIIDetector) -> List[TestResult]:
    ...
    for suite_name, test_cases, pii_type, min_f1 in test_suites:
        ...
        results.append(TestResult(
            name=suite_name,
            passed=passed,
            message=f"F1={f1:.3f} (min={min_f1})" if passed else
                    f"F1={f1:.3f} < {min_f1} required",
            precision=precision,
            recall=recall,
            f1=f1,
            details={
...
def run_edge_case_tests(detector: PIIDetector) -> List[TestResult]:
    ...
    results.append(TestResult(
        ...
        message="Correctly ignores obfuscated format" if len(matches) == 0 else
                "Incorrectly matched obfuscated email",
    ))
...
def main():
    ...
    parser.add_argument("--with-lilac", action="store_true",
                        help="Also test against Lilac library")
